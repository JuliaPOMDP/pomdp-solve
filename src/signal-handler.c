...
setUpIntervalTimer( int secs ) 
{
  ...
  */
   ...
   #ifdef _WIN32
     /* On Windows, create a timer that calls a specified function when the timer expires. */
     HANDLE hTimer = NULL;
     CreateTimerQueueTimer(&hTimer, NULL, (WAITORTIMERCALLBACK)signalHandlerSIGVTALRM, NULL, secs * 1000, 0, 0);
     if (hTimer == NULL) {
       Abort( "Trouble in function call to setUpIntervalTimer.\n");
     }
   #else
     /* On Unix-like systems, set an interval timer. */
...
#else
  #ifdef _WIN32
    /* On Windows, set the minimum and maximum working set sizes. */
    BOOL result = SetProcessWorkingSetSize(GetCurrentProcess(), mem_size_limit, mem_size_limit);
    if (!result) {
      fprintf(stderr, "Failed to set memory limit: %lu\n", GetLastError());
    }
  #else
    /* On Unix-like systems, set the virtual memory or data segment size limit. */
    struct rlimit rlp;
...
#ifdef _WIN32
  BOOL WINAPI signalHandlerSIGINT(DWORD dwCtrlType)
#else
...
#ifdef _WIN32
  SetConsoleCtrlHandler(signalHandlerSIGINT, TRUE);
#else
